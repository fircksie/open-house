...
function timeLabel(iso: string, timezone: string, includeDay = false) {
  return new Intl.DateTimeFormat("en-ZA", {
    ...(includeDay ? { weekday: "short" as const } : {}),
    hour: "2-digit",
    minute: "2-digit",
    hour12: false,
    timeZone: timezone,
...
function countryShort(country?: string) {
  return country ? country.slice(0, 3).toUpperCase() : "";
}
...
export function MatchCard({ match, timezone }: { match: TennisMatch; timezone: string }) {
  ...
  // Lock from actual match state, not from a provisional scheduled timestamp.
  ...
  return (
    <article className={`match-card ${isFav ? "has-favourite" : ""}`}>
      <button className="match-main" onClick={() => setOpen(!open)}>
        ...
        {[match.first, match.second].map((p, idx) => (
          <div className="player-row" key={p.id}>
            ...
            <div style={{ display: "flex", gap: 7 }}>
              {Array.from({ length: sets }).map((_, s) => (
                <span className="set-score" key={s}>
                  {idx === 0 ? match.sets[s]?.first : match.sets[s]?.second}
                </span>
              ))}
            </div>
            ...
            <div className="game">
              {match.state === "live"
                ? (
                  idx === 0 && match.gameScore
                    ? match.gameScore.split(/[–-]/)[0]?.trim()
                    : idx === 1 && match.gameScore
                      ? match.gameScore.split(/[–-]/)[1]?.trim()
                      : ""
                )
                : match.winnerPlayerId === p.id ? "✓" : ""}
            </div>
          </div>
        ))}
            ...
            {match.state === "live"
              ...
              : match.state === "completed"
                ...
                : !match.startsAt
                  ? "Start time TBC"
                  : scheduledTimePassed
                    ? `Awaiting start · scheduled ${localTime}`
                    : timezone === "America/New_York"
                      ? `${newYork} New York`
                      : `${localTime} local · ${newYork} New York`}
          </span>

          <span>
            {matchPicks.length
              ? `${matchPicks.length} family pick${matchPicks.length === 1 ? "" : "s"}`
              : isFav
                ? "Favourite player"
                : "Tap for details"}
          </span>
      ...
      {open && (
        <div className="details">
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 4, marginBottom: 10 }}>
            <button
              className={`star ${favorites.includes(match.first.id) ? "on" : ""}`}
              onClick={() => toggleFavorite(match.first.id)}
              aria-label={`Favourite ${match.first.name}`}
            >
              <Star size={19} fill={favorites.includes(match.first.id) ? "currentColor" : "none"} />
            </button>

            <button
              className={`star ${favorites.includes(match.second.id) ? "on" : ""}`}
              onClick={() => toggleFavorite(match.second.id)}
              aria-label={`Favourite ${match.second.name}`}
            >
              <Star size={19} fill={favorites.includes(match.second.id) ? "currentColor" : "none"} />
            </button>
          </div>

          <div className="edge">
            <div className="edge-top">
              <span className="edge-title">Open House Edge</span>
              <span className="chip">For fun · not odds</span>
            </div>

            {loadingEdge
              ? <div className="sub">Calculating from available ranking and form data…</div>
              : prediction?.enoughData
                ? (
                  <>
                    <div className="prob">
                      <div className="prob-side">
                        <strong>{prediction.firstProbability}%</strong>
                        <small>{match.first.name}</small>
                      </div>
                      <div className="prob-side">
                        <strong>{prediction.secondProbability}%</strong>
                        <small>{match.second.name}</small>
                      </div>
                    </div>
                    ...
                    <div className="factor-list">
                      {prediction.factors.map((f) => (
                        <div className="factor" key={f.label}>
                          <span>{f.label}</span>
                          <span>
                            {f.winnerPlayerId === match.first.id
                              ? match.first.name
                              : f.winnerPlayerId === match.second.id
                                ? match.second.name
                                : "Even"} · {f.detail}
                          </span>
                        </div>
          ...
          <div className="pick-area">
            <div className="pick-label">{locked ? "Family picks" : "Who wins?"}</div>

            <div className="pick-buttons">
              <button
                className={`pick ${userPick?.selected_player_id === match.first.id ? "selected" : ""}`}
                disabled={locked}
                onClick={() => submitPick(match, match.first.id, prediction?.underdogPlayerId === match.first.id)}
              >
                {match.first.name}
              </button>

              <button
                className={`pick ${userPick?.selected_player_id === match.second.id ? "selected" : ""}`}
                disabled={locked}
                onClick={() => submitPick(match, match.second.id, prediction?.underdogPlayerId === match.second.id)}
              >
                {match.second.name}
              </button>
            </div>
            ...
            {revealFamily && (
              <>
                <div className="family-counts">
                  <span>{match.first.name}: {count(match.first.id)}</span>
                  <span>{match.second.name}: {count(match.second.id)}</span>
                  <span>{members.length} players</span>
                </div>

                <div className="family-pick-list">
                  {matchPicks.map((p) => {
                    const member = members.find((m) => m.id === p.profile_id);
                    const chosen = p.selected_player_id === match.first.id ? match.first.name : match.second.name;
                    return <span key={p.id}>{member?.display_name ?? "Family"} → {chosen}</span>;
                  })}
                </div>
              </>
            )}
          ...
          <div className="reaction-area">
            <div className="pick-label">Match reaction</div>
            <div className="reaction-buttons">
              {REACTIONS.map((emoji) => (
                <button
                  key={emoji}
                  className={`reaction ${myReaction === emoji ? "selected" : ""}`}
                  onClick={() => reactToMatch(match.id, emoji)}
            ...
            {matchReactions.length > 0 && (
              <div className="reaction-list">
                {matchReactions.map((r) => {
                  const member = members.find((m) => m.id === r.profile_id);
                  return <span key={`${r.profile_id}-${r.match_id}`}>{member?.display_name ?? "Family"} {r.emoji}</span>;
                })}
              </div>
            )}
          </div>
