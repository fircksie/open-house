...
export function FamilyView(){
  ...
  return <main><header className="page-head"><div className="eyebrow">Fircks US Open</div><h2>Family standings</h2><div className="sub">One point for a correct pick. A correct Open House underdog pick earns one bonus point.</div></header>
    <section className="card"><div className="leaderboard">{rows.map((r,i)=><div className="leader" key={r.m.id}><div className="position">{i+1}</div><div><div className="leader-name">{r.m.display_name}{r.m.id===profile?.id?" · you":""}</div><div className="sub">{r.correct}/{r.total} picks · {r.accuracy}% · streak {r.current}</div></div><div className="leader-stat">{r.points} pts<small>{r.upsets} upsets</small></div></div>)}</div></section>
    <section className="section"><div className="card"><div className="section-title">How it works</div><div className="sub" style={{marginTop:10}}>Picks lock at the scheduled start. Once the match result appears in the feed, the app settles every family pick for that match. The leaderboard uses the stored result, so everyone sees the same score.</div>{mode==="local"&&<div className="sub" style={{marginTop:10}}>You are currently in local demo family mode. Add Supabase environment variables to make this shared across devices.</div>}</div></section>
  </main>
}
